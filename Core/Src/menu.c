...
void (*logic[NUM_ITEMS][NUM_BTN_COMBINATIONS][NUM_OF_PRESS_TYPES])(void);
...
void init_menu_functions(void) {
	for(int item = 0; item < NUM_ITEMS; ++item) {
		for(int mask = 0; mask < NUM_BTN_COMBINATIONS; ++mask) {
			for(int press = 0; press < NUM_OF_PRESS_TYPES; ++press) {
				logic[item][mask][press] 	= NULL;
			}
		}
		logic[item][MASK_ENTER]	[CLICK] = numbers_on_off; //item_selection_cycle;
		logic[item][MASK_ENTER]	[HOLD] = item_selection_cycle;

		logic[item][MASK_LEFT]	[CLICK] = item_goto_prev; // Goes to a previous item when "is_selected == 0" and decreases parameter
		logic[item][MASK_RIGHT]	[CLICK] = item_goto_next; // Goes to a previous item when "is_selected == 0" and decreases parameter
		logic[item][MASK_ENTER]	[CLICK] = item_selection_cycle;
		logic[item][MASK_ENTER]	[CLICK] = item_selection_cycle;
//		logic[item][MASK_ENTER]	[HOLD] 	= confirm_parameter_value;
	}

	logic[ITEM_ADJUST_YEAR][MASK_LEFT]		[CLICK] = year_decrease; // Goes to a previous item when "is_selected == 0" and decreases parameter
	logic[ITEM_ADJUST_YEAR][MASK_RIGHT]		[CLICK] = year_increase;

	logic[ITEM_ADJUST_DATE][MASK_LEFT]	[CLICK] = days_decrease; // Goes to a previous item when "is_selected == 0" and decreases parameter
	logic[ITEM_ADJUST_DATE][MASK_RIGHT][CLICK] = days_increase;
	logic[ITEM_ADJUST_MONTH][MASK_LEFT]	[CLICK] = month_decrease; // Goes to a previous item when "is_selected == 0" and decreases parameter
	logic[ITEM_ADJUST_MONTH][MASK_RIGHT][CLICK] = month_increase;

	logic[ITEM_ADJUST_HOUR][MASK_LEFT]	[CLICK] = hours_decrease; // Goes to a previous item when "is_selected == 0" and decreases parameter
	logic[ITEM_ADJUST_HOUR][MASK_RIGHT][CLICK] = hours_increase;
	logic[ITEM_ADJUST_MINUTE][MASK_LEFT]	[CLICK] = minutes_decrease; // Goes to a previous item when "is_selected == 0" and decreases parameter
	logic[ITEM_ADJUST_MINUTE][MASK_RIGHT][CLICK] = minutes_increase;
}
