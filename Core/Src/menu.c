...
Item_TypeDef* 			items;
ItemType				item_parent	[NUM_OF_ITEMS];	// Items lists, describes menu hierarchy: 	up one level,
ItemType				item_child	[NUM_OF_ITEMS];	//											down one level (submenu),
ItemType				item_prev	[NUM_OF_ITEMS];	//											previous item,
ItemType				item_next	[NUM_OF_ITEMS];	//
...
static inline void init_items_hierarchy_horizontal(uint16_t num_of_menus) {
	...
	for(ItemType current = 1; current < num_of_items; ++current) {
		for(ItemType prev = 1; prev < num_of_items; ++prev) {
			if(menu->next[prev] == current)
				menu->prev[current] = prev;
		}
	}
}
